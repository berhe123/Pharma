...
namespace Pharma
{
    class CDigits
    {
        ...
        public static void AllowUIntegerOnly(TextBox txtUInt)
        {
            ...
            {
                //j = txtUInt.TextLength;
                //txtUInt.Select(j - 1, j);
                //txtUInt.SelectedText = "";
                if (txtUInt.Tag != null)
        ...
        public static void AllowDoubleOnly(TextBox txtDbl)
        {
            double i;
            //int j;
            if (!double.TryParse(txtDbl.Text, out i) & txtDbl.TextLength > 0)
            {
                //j = txtDbl.TextLength;
                //txtDbl.Select(j - 1, j);
                //txtDbl.SelectedText = "";
                if (txtDbl.Tag != null)
                    txtDbl.Text = txtDbl.Tag.ToString();
                else
                    txtDbl.Text = string.Empty;
            }
            else
                txtDbl.Tag = txtDbl.Text;
        }
