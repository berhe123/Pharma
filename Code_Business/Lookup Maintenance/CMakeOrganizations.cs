...
namespace Pharma
{
    class CMakeOrganizations
    {
        public static List<CMakeOrganizations> ItemCategoriesCollection = null;
        ...
        public CMakeOrganizations()
        {
            _ConnectionString = ConnectionString;
        }
        public CMakeOrganizations(int Id, string Name, int Categorytype)
        {
            _Id = Id;
            _Name = Name;
        }
        ...
        public DataTable GetRecord()
        {
            ...
            command.CommandText = "Select * from tblMakeOrganizations where Id=@Id";
            command.CommandType = CommandType.Text;
            DataTable dTable = new DataTable("tblMakeOrganizations");
        ...
        public DataTable GetRecords()
        {
            SqlConnection connection = new SqlConnection(_ConnectionString);
            SqlCommand command = new SqlCommand();
            command.CommandText = "Select * from tblMakeOrganizations";
            command.CommandType = CommandType.Text;
            DataTable dTable = new DataTable("tblMakeOrganizations");
            SqlDataAdapter adapter = new SqlDataAdapter(command);
            command.Connection = connection;
        ...
        public bool Insert()
        {
            SqlConnection connection = new SqlConnection(_ConnectionString);
            SqlCommand command = new SqlCommand();
            command.CommandText = "Insert Into tblMakeOrganizations" +
                                   " (Name,UserId,ComputerName,EventSource)" +
                                   " Values (@Name,@UserId,@ComputerName,@EventSource)" +
                                   " SELECT @Id=Max(id) from tblMakeOrganizations";
            command.CommandType = CommandType.Text;
            command.Connection = connection;
            try
            {
            ...
            {
                throw new Exception("CMakeOrganizations::Insert::Error! " + ex.Message, ex);
            }
            finally
            {
                if (connection != null) connection.Close();
        ...
        public bool Update()
        {
            SqlConnection connection = new SqlConnection(_ConnectionString);
            SqlCommand command = new SqlCommand();
            command.CommandText = "Update tblMakeOrganizations" +
                                   " Set Name = @Name," +
                                   " UserId =@UserId," +
        ...
        public bool Delete()
        {
            SqlConnection connection = new SqlConnection(_ConnectionString);
            SqlCommand command = new SqlCommand();
            command.CommandText = "Delete From tblMakeOrganizations Where Id = @Id ";
            command.CommandType = CommandType.Text;
            command.Connection = connection;
        ...
        public int Exists(bool bNew)
        {
            ...
            if (bNew)
                command.CommandText = "SELECT Id FROM [dbo].[tblMakeOrganizations] WHERE " +
                                       " Name = @Name";
            else
                command.CommandText = "SELECT Id FROM [dbo].[tblMakeOrganizations] WHERE " +
                                       " Name = @Name " +
        ...
        public List<CMakeOrganizations> GetList()
        {
            List<CMakeOrganizations> RecordsList = new List<CMakeOrganizations>();
            SqlConnection connection = new SqlConnection(_ConnectionString);
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    CMakeOrganizations objCItemCategories = new CMakeOrganizations();
